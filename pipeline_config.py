...
#
# Every step script previously hardcoded absolute paths from the
# original development machine. This module centralizes them and adds a BALANCE_MODE switch so the
...
#                         (default: <repo>/data/custom)
#   ZSH_ELLIPTIC_ROOT  - folder containing the Elliptic CSVs
#                         (default: <repo>/data/elliptic)
...
# Repository root, so a fresh clone resolves paths without configuration.
_REPO = os.path.dirname(os.path.abspath(__file__))

DATA_ROOT = os.environ.get(
    "ZSH_DATA_ROOT", os.path.join(_REPO, "data", "custom")
)
OUTPUT_ROOT = os.environ.get("ZSH_OUTPUT_ROOT", _REPO)
ELLIPTIC_ROOT = os.environ.get(
    "ZSH_ELLIPTIC_ROOT", os.path.join(_REPO, "data", "elliptic")
)
...
PIPELINE_PYTHON = os.environ.get("ZSH_PYTHON", sys.executable)
# Directory containing the step scripts themselves (cwd for subprocess.run).
PIPELINE_WORK_DIR = os.environ.get(
    "ZSH_WORK_DIR", os.path.dirname(os.path.abspath(__file__))
)
