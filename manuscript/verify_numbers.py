...
def _results_dir():
    """$ZSH_RESULTS, else ../results when this lives in manuscript/."""
    env = os.environ.get("ZSH_RESULTS")
    if env:
        return Path(env)
    if (HERE.parent / "results").is_dir():
        return HERE.parent / "results"
    raise SystemExit("Set ZSH_RESULTS to the results directory, or run this from "
                     "manuscript/ inside the repository.")


RES = _results_dir()
TEXT = (HERE / "manuscript.md").read_text(encoding="utf-8")
SUPP = HERE / "supplementary.md"          # numbers may live in the supplementary file
if SUPP.exists():
...
# ---------------------------------------------------------------- ceiling, oracle, matching (E6, E15, E16)
ct = csv("E6/test_independent.csv")
cz = ct[ct.method == "ZSH"].set_index("target")
for t in ("L2:P2PKH", "L2:P2TR", "L2:P2WPKH", "L3:runes", "L4:exchange", "L2:P2SH"):
    # the table prints one decimal, the text rounds to whole percent
    check(f"attained share {t}", pct(cz.loc[t, "ap"]), f"{100 * cz.loc[t, 'ap']:.1f}",
          f"{round(100 * cz.loc[t, 'ap'])}%")
e8c = csv("E8/concentration.csv")
check("Elliptic attained", pct(e8c[(e8c.setting == "AF-165") & (e8c.method == "ZSH")].ap.iloc[0], 1))
check("random forest attained", pct(js("E8/summary.json")["random_forest_reference"]["ap"], 0))
o15 = csv("E15/oracle_concentration.csv").pivot(index="method", columns="target", values="ap_lift")
check("oracle P2PKH", f"{o15.loc['oracle rank-power: L2:P2PKH', 'L2:P2PKH']:.1f}")
check("oracle P2SH", f"{o15.loc['oracle rank-power: L2:P2SH', 'L2:P2SH']:.1f}")
check("oracle exchange", f"{o15.loc['oracle rank-power: L4:exchange', 'L4:exchange']:.1f}")
check("E15 reference P2PKH", f"{o15.loc['reference (unsupervised)', 'L2:P2PKH']:.1f}")
m16 = csv("E16/matching.csv").set_index(["period", "method"])
check("matching median test", f"{m16.loc[('Test', 'ZSH'), 'median_jaccard']:.2f}")
check("matching median prospective", f"{m16.loc[('Prospective', 'ZSH'), 'median_jaccard']:.2f}")
check("K-means++ matched at 0.5", str(int(m16.loc[("Test", "K-means++"), "matched_0.5"])))
check("K-means++ matched share", pct(m16.loc[("Test", "K-means++"), "share_matched_0.5"], 0))

# ---------------------------------------------------------------- external labels (E13, E14)
s13 = js("E13/summary.json")
check("actor graph largest component", f"{s13['components']['largest_component_transactions']:,}")
check("transactions with address data", f"{s13['components']['transactions_with_addresses']:,}")
check("random address split share", pct(s13["random_address_split"]["clean_share_mean"]))
check("actor-disjoint training set", f"{s13['holdout']['training_actor_disjoint']:,}")
check("training with addresses", f"{s13['holdout']['training_with_addresses']:,}")
check("actor-disjoint share", pct(s13["holdout"]["training_actor_disjoint_share"]))
check("actor-holdout evaluation set", f"{s13['holdout']['evaluation_transactions']:,}")
a13 = csv("E13/actor_holdout_concentration.csv").set_index("method")
check("actor-holdout ZSH AP lift", f"{a13.loc['ZSH', 'ap_lift']:.2f}")
check("actor-holdout ZSH CI", f"{a13.loc['ZSH', 'ap_lift_lo']:.2f}–{a13.loc['ZSH', 'ap_lift_hi']:.2f}")
check("actor-holdout ZSH precision", pct(a13.loc["ZSH", "prec@0.25"]))
cj = csv("E14/coinjoin_source.csv").set_index("period")
check("known CoinJoins in test", str(int(cj.loc["Test", "known_coinjoins"])))
check("known CoinJoins in prospective", str(int(cj.loc["Prospective", "known_coinjoins"])))
check("count rule recall (test)", pct(cj.loc["Test", "count_rule_recall"]))
check("equal-output recall", pct(cj.loc["Prospective", "equal_output_recall"]))
s14 = js("E14/summary.json")
check("external CoinJoin list size", f"{s14['source']['transactions']:,}")
check("profiles holding labelled CoinJoins", str(s14["profiles"]["Test"]["profiles_used"]))
check("largest CoinJoin profile share", pct(s14["profiles"]["Test"]["largest_share"], 0))
